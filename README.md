...
In its current state there are no documented steps to update the `Services.jl` file.
...
[`AWSSDK.jl`](https://github.com/JuliaCloud/AWSSDK.jl) is a package which contains high-level API definitions for all Amazon Services.
...
Since these files contain a large amount of functions, including them in the `AWSCore` module would take a substantial amount of time.
Instead it will be used by calling a `macro` to generate the module and include the service file when needed.
...
The [`JavaScript SDK`](https://github.com/aws/aws-sdk-js/tree/master/apis) is the most simple to parse as all service definitions are defined as JSON files while other SDKs define them on a per language basis.
...
- [`JuliaCloud/AWSSDK.jl`](https://github.com/JuliaCloud/AWSSDK.jl)
